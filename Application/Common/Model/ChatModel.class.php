...
class ChatModel extends \Think\Model
{
	...
	//根据订单查询聊天记录
	public function listbyOrderid($orderid,$ordertype,$orderby='ASC',$offset=NULL)
	{
		if(empty($orderid) || empty($ordertype)){
			return array();
		}
		if(empty($offset)){
			$list = $this->table('tw_chat')->where(array('orderid'=>$orderid,'ordertype'=>$ordertype))->order('addtime '.$orderby)->select();
		}else{
			$list = $this->table('tw_chat')->where(array('orderid'=>$orderid,'ordertype'=>$ordertype))->order('addtime '.$orderby)->limit($offset,1)->select();
		}
		return $list;
	}
	//根据广告查询聊天记录
	public function listbyAdvid($advid,$advtype,$userid,$orderby='ASC',$offset=0){
		...
		if(empty($offset)){
			$list = $this->table('tw_chat')->where("advid=$advid and advtype=$advtype and (fromuid=$userid or touid=$userid)")->order('addtime '.$orderby)->select();
		}else{
			$list = $this->table('tw_chat')->where("advid=$advid and advtype=$advtype and (fromuid=$userid or touid=$userid)")->order('addtime '.$orderby)->limit($offset,1)->select();
		}
		return $list;
	}
	
	//添加一条记录
	public function addRecord($fromuid, $touid, $orderid, $ordertype, $content='', $addon = NULL, $advid, $advtype, $newtype=0){
		...
		$data['isread'] = 0;
		$data['type'] = $newtype;//dump($data);die;
	...
	public function getCount($orderid){
		...
		$count = $this->table('tw_chat')->where(array('orderid'=>$orderid, 'fromuid'=>1, 'touid'=>1))->count();
		return $count;
	}
