...
function send_moble($moble, $content)
{
	...
	$user = 'zhanghao'; //短信平台帐号
	$pass = md5('mima'); //短信平台密码
	$time=$_SESSION['time'];
	...
	$_SESSION['time'] = time();
	$name = 'TOKENVOTO';//网站的名称
	$content = "【TOKENVOTO】".$content;
...
function chkAuthCache()
{
	...
	debug('A');
	$md2 = md5(md5($key1) + md5('mollymobi') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
	...
	debug('D');
	$times--;
	$md = md5(md5($key1) + md5('mollymobi') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
...
function mollymobi_getCoreConfig(){	
	$file_path = DATABASE_PATH .'fenqubiao.json';
	if (file_exists($file_path)) {
		$mollymobi_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$mollymobi_CoreConfig = json_decode($mollymobi_CoreConfig, true);
		$SystemQQ = base64_decode("VG9rZW5GYWN0b3J5");
		if($mollymobi_CoreConfig['bcxsscom']==$SystemQQ){
			return $mollymobi_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/mollymobi/log.html";
...
function gettrade($id) {
	$trust = M('User')->where(array("id"=>$id))->getField('trade_id');
	if($trust) {
		$trust = trim($trust, ',');//去除前面的逗号
		$trust_arr = explode(",", $trust);
		return count($trust_arr);
...
function gettrademoney($id) {
	return M('order_buy')->where(array('buy_id|sell_id'=>$id, 'status'=>array('in', array(3, 4))))->sum('deal_amount');
}
//判断现在是否在开放时间返回1显示0隐藏
...
function truspingbi($id){
	$info=M('user')->Field('id,xinren,pingbi')->where(array('id'=>$id))->find();
	return $info;
}
...
function getname($id){
	$name=M('user')->Field('username')->where("id=".$id)->find();
...
function getinfo($id){
	$info=M('user')->Field('id,username,headimg')->where("id=".$id)->find();
	return $info;
}
function getcoinname($id) {
	...
	return M('coin')->where('id='.$id)->getField('title');
}
...
function timediff($begin_time, $end_time){
	...
	if($days == 0) {
		return $res['hour'].'时'.$res['min'].'分'.$res['sec'].'秒';
	}else{
