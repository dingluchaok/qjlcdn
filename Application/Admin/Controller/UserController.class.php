...
class UserController extends AdminController
{
	...
	public function apply_seller($field = NULL, $name = NULL, $status = NULL)
	{
		$where = array();
		...
		$count = M('seller_apply')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M('seller_apply')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
	...
	public function do_seller($id, $status) {
		//dump($status);die;
		$find = M('seller_apply')->where(array('id'=>$id))->find();
		if(empty($find) || $find['status'] != 0) {
			$this->error('参数错误');
		}		
		if($status == 1) {
			//通过
			$rs = M('seller_apply')->where(array('id'=>$id))->setField('status', $status);
			if($rs) {
				$this->success('操作成功');
			}else{
				$this->error('操作失败');
		...
		if($status == 2) {
			...
			$mo = M();
			$mo->startTrans();
			$rs3 = $mo->table('mollymobi_seller_apply')->where(array('id'=>$id))->setField('status', $status);
	...
	public function edit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if (isset($_POST['id'])) {
                ...
                $mo->execute('lock tables mollymobi_user write , mollymobi_user_coin write ');
                $rs[] = $mo->table('mollymobi_user')->add($_POST);
                $rs[] = $mo->table('mollymobi_user_coin')->add(array('userid' => $rs[0]));
                $flag = true;
            }
	...
	public function admin($name = NULL, $field = NULL, $status = NULL)
	{
		...
		if (!in_array('email', $DbFields)) {
			M()->execute('ALTER TABLE `mollymobi_admin` ADD COLUMN `email` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
		}
...
/* 		$l_table = $prefix . 'ucenter_member';//Common\Model\AuthGroupModel::MEMBER;
	...
	public function logEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('mollymobi_user_log')->where(array('id'=>$id))->save($_POST)) {
                    $this->success('编辑成功！');
                } else {
                    $this->error('编辑失败！');
                }
            } else {
                if (M()->table('mollymobi_user_log')->add($_POST)) {
                    $this->success('添加成功！');
                } else {
                    $this->error('添加失败！');
                }
	...
	public function qianbaoEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('mollymobi_user_qianbao')->where(array('id' => $id))->save($_POST)) {
                    $this->success('编辑成功！');
                } else {
                    $this->error('编辑失败！');
                }
            } else {
                if (M()->table('mollymobi_user_qianbao')->add($_POST)) {
                    $this->success('添加成功！');
                } else {
                    $this->error('添加失败！');
                }
	...
	public function bankEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                if (M()->table('mollymobi_user_bank')->where(array('id' => $id))->save($_POST)) {
                    $this->success('编辑成功！');
                }
                else {
                    $this->error('编辑失败！');
                ...
                if (M()->table('mollymobi_user_bank')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
	...
	public function coin($name = NULL, $field = NULL, $order = 0)
	{
		$where = array();
		...
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M('UserCoin')->where($where)->order($order)->limit($Page->firstRow . ',' . $Page->listRows)->select();
	...
	public function ccoin($name = NULL, $field = NULL)
	{
		$where = array();
	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                ...
                if (M()->table('mollymobi_user_coin')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
	...
	public function goodsEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('mollymobi_user_goods')->where(array('id'=>$id))->save($_POST)) {
                    $this->success('编辑成功！');
                }
                else {
                    $this->error('编辑失败！');
                ...
                if (M()->table('mollymobi_user_goods')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
