...
class ShopController extends AdminController
{
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables mollymobi_user_coin write,mollymobi_shop_log write');
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('mollymobi_shop_log')->where(array('id' => $id))->save(array('status' => 2));
