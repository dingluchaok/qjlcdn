...
class FinanceController extends AdminController
{
	...
	public function myczQueren()
	{
		...
		$mo->execute('lock tables mollymobi_user_coin write,mollymobi_mycz write,mollymobi_finance write,mollymobi_invit write,mollymobi_user write');
		$rs = array();
		$finance = $mo->table('mollymobi_finance')->where(array('userid' => $mycz['userid']))->order('id desc')->find();
		$finance_num_user_coin = $mo->table('mollymobi_user_coin')->where(array('userid' => $mycz['userid']))->find();
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
		$rs[] = $mo->table('mollymobi_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 2, 'mum' => $mycz['num'], 'endtime' => time()));
		$finance_mum_user_coin = $mo->table('mollymobi_user_coin')->where(array('userid' => $mycz['userid']))->find();
		$finance_hash = md5($mycz['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mycz['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.mollymobi.com');
		...
		$cur_user_info = $mo->table('mollymobi_user')->where(array('id' => $mycz['userid']))->find();
		...
		if($cur_user_info['invit_1']&&$cur_user_info['invit_1']>0&&1==2){
			...
			if ($invit_1_jiner) {
				//处理前信息
				$finance_1 = $mo->table('mollymobi_finance')->where(array('userid' => $cur_user_info['invit_1']))->order('id desc')->find();
		        $finance_num_user_coin_1 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();
				
				//开始处理
				$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->setInc('cny',$invit_1_jiner);
				$rs[] = $mo->table('mollymobi_invit')->add(array('userid' => $cur_user_info['invit_1'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '一代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_1_jiner, 'addtime' => time(), 'status' => 1));
				
				//处理后
				$finance_mum_user_coin_1 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();
				$finance_hash_1 = md5($cur_user_info['invit_1'].$finance_num_user_coin_1['cny'] . $finance_num_user_coin_1['cnyd'] . $invit_1_jiner . $finance_mum_user_coin_1['cny'] . $finance_mum_user_coin_1['cnyd'] . MSCODE . 'auth.mollymobi.com');
		...
		if($cur_user_info['invit_2']&&$cur_user_info['invit_2']>0&&1==2){
			...
			if ($invit_2_jiner) {
				...
				//处理前信息
				$finance_2 = $mo->table('mollymobi_finance')->where(array('userid' => $cur_user_info['invit_2']))->order('id desc')->find();
		        $finance_num_user_coin_2 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();
				
				//开始处理
				
				$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->setInc('cny',$invit_2_jiner);
				$rs[] = $mo->table('mollymobi_invit')->add(array('userid' => $cur_user_info['invit_2'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '二代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_2_jiner, 'addtime' => time(), 'status' => 1));
			
				//处理后
				$finance_mum_user_coin_2 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();
				$finance_hash_2 = md5($cur_user_info['invit_2'].$finance_num_user_coin_2['cny'] . $finance_num_user_coin_2['cnyd'] . $invit_2_jiner . $finance_mum_user_coin_2['cny'] . $finance_mum_user_coin_2['cnyd'] . MSCODE . 'auth.mollymobi.com');
		...
		if($cur_user_info['invit_3']&&$cur_user_info['invit_3']>0&&1==2){
			...
			if ($invit_3_jiner) {
				...
				//处理前信息
				$finance_3 = $mo->table('mollymobi_finance')->where(array('userid' => $cur_user_info['invit_3']))->order('id desc')->find();
		        $finance_num_user_coin_3 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();
				
				//开始处理

				$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->setInc('cny',$invit_3_jiner);
				$rs[] = $mo->table('mollymobi_invit')->add(array('userid' => $cur_user_info['invit_3'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '三代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_3_jiner, 'addtime' => time(), 'status' => 1));
			
				//处理后
				$finance_mum_user_coin_3 = $mo->table('mollymobi_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();
				$finance_hash_3 = md5($cur_user_info['invit_3'].$finance_num_user_coin_3['cny'] . $finance_num_user_coin_3['cnyd'] . $invit_3_jiner . $finance_mum_user_coin_3['cny'] . $finance_mum_user_coin_3['cnyd'] . MSCODE . 'auth.mollymobi.com');
	...
	public function rgbQueren()
	{
		...
		$mo = M();
		$mo->execute("set autocommit=0");
		$mo->execute("lock tables mollymobi_user_coin write,mollymobi_myzr write");
		$rs = array();
		$rs[] = $mo->table("mollymobi_user_coin")->where(array("userid" => $mycz["userid"]))->setInc($mycz['coinname'], $mycz["num"]);
		$rs[] = $mo->table("mollymobi_myzr")->where(array("id" => $mycz["id"]))->save(array("status" => 1, "mum" => $mycz["num"], "endtime" => time()));
		if (check_arr($rs)) {
			$mo->execute("commit");
			$mo->execute("unlock tables");
			$this->success("操作成功！");
	...
	public function myzcQueren($id = NULL)
	{
		...
		if ($zhannei) {
			$rs[] = $mo->table('mollymobi_myzr')->add(array('userid' => $zhannei['userid'], 'username' => $myzc['username'], 'coinname' => $coin, 'txid' => md5($myzc['username'] . $user_coin[$coin . 'b'] . time()), 'num' => $myzc['num'], 'fee' => $myzc['fee'], 'mum' => $myzc['mum'], 'addtime' => time(), 'status' => 1));
			$rs[] = $r = $mo->table('mollymobi_user_coin')->where(array('userid' => $zhannei['userid']))->setInc($coin, $myzc['mum']);
		}
		...
		if (0 < $myzc['fee']) {
			$rs[] = $mo->table('mollymobi_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $myzc['num'], 'fee' => $myzc['fee'], 'mum' => $myzc['mum'], 'type' => 2, 'addtime' => time(), 'status' => 1));

			if ($mo->table('mollymobi_user_coin')->where(array($coin . 'b' => $Coin['zc_user']))->find()) {
				$rs[] = $mo->table('mollymobi_user_coin')->where(array($coin . 'b' => $Coin['zc_user']))->setInc($coin, $myzc['fee']);
				debug(array('lastsql' => $mo->table('mollymobi_user_coin')->getLastSql()), '新增费用');
			}
			else {
				$rs[] = $mo->table('mollymobi_user_coin')->add(array($coin . 'b' => $Coin['zc_user'], $coin => $myzc['fee']));
			}
		}
