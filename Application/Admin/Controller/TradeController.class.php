...
class TradeController extends AdminController
{
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		foreach($list as $k=>$v){
			...
			$list[$k]['tradetimemollymobi'] = $begintradeqq_3479015851."-".$endtradeqq_3479015851;
		}
	...
	public function marketEdit($id = NULL)
	{
		...
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$market_mollymobi = M('Market')->where(array('id' => $id))->find();
				$this->data = $market_mollymobi;

				if($market_mollymobi['begintrade']){
					$beginshi = explode(":",$market_mollymobi['begintrade'])[0];
					$beginfen = explode(":",$market_mollymobi['begintrade'])[1];
				}else{
					$beginshi = "00";
					$beginfen = "00";
				}
				
				if($market_mollymobi['endtrade']){
					$endshi = explode(":",$market_mollymobi['endtrade'])[0];
					$endfen = explode(":",$market_mollymobi['endtrade'])[1];
				}else{
					$endshi = "23";
					$endfen = "59";
				}
			...
			$this->assign('mollymobi_getCoreConfig',$mollymobi_getCoreConfig['mollymobi_indexcat']);
