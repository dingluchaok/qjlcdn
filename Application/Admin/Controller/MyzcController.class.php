...
class MyzcController extends AdminController
{
	...
	public function refuse($id = NULL) {
		...
		$rs = $this->Model->where(array("id" => trim($id)))->save(array("status" => 2));
		if ($rs) {
			$rs2 = M('user_coin')->where(array('userid'=>$myzc['userid']))->setDec($myzc["coinname"].'d', $myzc['mum']);
			$rs3 = M('user_coin')->where(array('userid'=>$myzc['userid']))->setInc($myzc["coinname"], $myzc['mum']);
			if($rs2 && $rs3) {
				$this->success("确认成功！");
			}else{
				$this->error('状态修改成功，返币失败');
			}
