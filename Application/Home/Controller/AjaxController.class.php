...
class AjaxController extends HomeController
{
	public function checkorder(){
		...
		$map['sell_id|buy_id'] = userid();
		$map['status'] = array('in', array(0, 1));
		$order = M('order_buy')->where($map)->select();
		if(!empty($order)){
			$data['status'] = 1;
	...
	public function searchTrade($ele = '') {
		$ele = strtolower($ele);
	...
	public function orderChat($orderid,$ordertype,$chatnum,$status=NULL){
		...
		if(!empty($status) && $status == 3){
			$list = $module->listbyOrderid($orderid,3,'ASC',$chatnum);
		}else{
			$list = $module->listbyOrderid($orderid,$ordertype,'ASC',$chatnum);
		}
		foreach($list as $k=>$v){
	...
	public function upauth($img_1, $img_2, $img_3) {
		if(!userid()) {
			$this->error('请登录操作');
		}
		...
		$path = $img_1.'_'.$img_2.'_'.$img_3;
		$rs = M('User')->where(array('id' => userid()))->save(array('idcardimg1' => $path));
		if($rs !== false) {
			$this->success('提交成功');
		}else{
	...
	public function wechatimage()
	{
		...
		if(!empty($user['c_wechat_img'])) {
			//echo 'img';exit();
		}
		$upload = new \Think\Upload();
		$upload->maxSize = 3145728;
		$upload->exts = array('jpg','png');
		$upload->rootPath = './Upload/wechat/';
		$upload->autoSub = false;
		$info = $upload->upload();
		if(!$info) {
			echo "error";
	...
	public function alipayimage()
	{
		if(!userid()) {
			echo 'error';exit();
		}
		$user = M('user')->where('id='.userid())->find();
		if(!empty($user['c_alipay_img'])) {
			//echo 'img';exit();
		}
		$upload = new \Think\Upload();
		$upload->maxSize = 3145728;
		$upload->exts = array('jpg','png');
		$upload->rootPath = './Upload/alipay/';
		$upload->autoSub = false;
		$info = $upload->upload();//dump($upload->getError());
		if(!$info) {
			echo "error";
			exit();
		}
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		$data = (APP_DEBUG ? null : S('mollymobi_getTopCoinMenu'));
		...
		if (!$data) {
			...
			foreach($mollymobi_getCoreConfig['mollymobi_indexcat'] as $k=>$v){
				$data[$k][title] = $v;
			}
			...
			S('mollymobi_getTopCoinMenu', $data);
		}
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	{
		//$data = (APP_DEBUG ? null : S('mollymobi_allcoin'));
		
		
		$mollymobi_data=array();
		
		$mollymobi_data['info']="数据异常";
		$mollymobi_data['status']=0;
		$mollymobi_data['url']="";
		...
		if (!$data) {
			...
			$mollymobi_data['info']="数据正常";
			$mollymobi_data['status']=1;
			$mollymobi_data['url']="";
			...
			foreach (C('market') as $k => $v) {
				...
				if($id==1){
					if(stristr($k,'_cny')){
						$mollymobi_data['url'][$k][0] = $v['title'];
						$mollymobi_data['url'][$k][1] = round($v['new_price'], $v['round']);
						$mollymobi_data['url'][$k][2] = round($v['buy_price'], $v['round']);
						$mollymobi_data['url'][$k][3] = round($v['sell_price'], $v['round']);
						$mollymobi_data['url'][$k][4] = isset($themarketLogs[$k]) ? $themarketLogs[$k] : 0;//round($v['volume'] * $v['new_price'], 2) * 1;
						$mollymobi_data['url'][$k][5] = '';
						$mollymobi_data['url'][$k][6] = round($v['volume'], 2) * 1;
						$mollymobi_data['url'][$k][7] = round($v['change'], 2);
						$mollymobi_data['url'][$k][8] = $v['name'];
						$mollymobi_data['url'][$k][9] = $v['xnbimg'];
						$mollymobi_data['url'][$k][10] = '';
					}
				}
				if($id==2){
					if(stristr($k,'_btc')){
		...
		if ($ajax) {
			echo json_encode($mollymobi_data);
			unset($mollymobi_data);
			exit();
		}
		else {
			return $mollymobi_data;
		}
	}
...
/*         if ($marketLogs) {
		...
		if (!$data) {
			...
			foreach (C('market') as $k => $v) {
				...
				if($v['jiaoyiqu']==$id){
						...
						$mollymobi_data['url'][$k][9] = $v['xnbimg'];
						$mollymobi_data['url'][$k][10] = '';
						$mollymobi_data['url'][$k][11] = strtoupper(explode('_', $v['name'])[0]);
						$mollymobi_data['url'][$k][12] = strtoupper(explode('_', $v['name'])[1]);
				}
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if(!$mollymobi_getCoreConfig){
			...
			$mollymobi_putong = $mollymobi_getCoreConfig['mollymobi_userTradeNum'];
			$mollymobi_teshu = $mollymobi_getCoreConfig['mollymobi_specialUserTradeNum'];
		}
		...
		if (!$data_getDepth[$market][$trade_moshi]) {
			...
			if (($trade_moshi == 3) || ($trade_moshi == 4)) {
				...
				if(userid()){
					...
					if($usertype ==1){
						$limt = $mollymobi_teshu;
					}else{
						$limt = $mollymobi_putong;
					}
				}else{
					$limt = $mollymobi_putong;
				}
			...
			$mo = M();
			if ($trade_moshi == 1) {
				$buy = $mo->query('select id,price,sum(num-deal)as nums from mollymobi_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit ' . $limt . ';');
				$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from mollymobi_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit ' . $limt . ';'));
			}
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		$result = M()->query('select id,price,num,deal,mum,type,fee,status,addtime from mollymobi_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
		...
		$tradeLog = M('TradeLog')->where(array('status' => 1, 'market' => $market,'userid'=>userid()))->order('id desc')->limit(10)->select();
		if ($tradeLog) {
			foreach ($tradeLog as $k => $v) {
				$data['mytrade'][$k]['addtime'] = date('m-d H:i:s', $v['addtime']);
				$data['mytrade'][$k]['type'] = $v['type'];
				$data['mytrade'][$k]['price'] = $v['price'] * 1;
				$data['mytrade'][$k]['num'] = round($v['num'], 6);
				$data['mytrade'][$k]['id'] = round($v['id']);
				$data['mytrade'][$k]['mum'] = round($v['mum'], 2);
			}
		}else{
			$data['mytrade'] = null;
		}
