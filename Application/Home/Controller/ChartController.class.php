...
class ChartController extends HomeController
{
	public function getJsonData($market = NULL, $ajax = 'json')
	{
		if ($market) {
			...
			if(!$mollymobi_getCoreConfig){
				...
				$mollymobi_putong = $mollymobi_getCoreConfig['mollymobi_userTradeDetailNum'];
				$mollymobi_teshu = $mollymobi_getCoreConfig['mollymobi_specialUserTradeDetailNum'];
			}
			
			$limit = $mollymobi_putong;
			if(userid()){
				$usertype = M('User')->where(array($id => $userid))->getField('usertype');
				if($usertype ==1){
					$limit = $mollymobi_teshu;
				}else{
					$limit = $mollymobi_putong;
				}
			}
	...
	protected function getTradeBuy($market,$limit)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from mollymobi_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit '.$limit);
		$data = '';
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from mollymobi_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
