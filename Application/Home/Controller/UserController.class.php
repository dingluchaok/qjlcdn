...
class UserController extends HomeController
{
	...
	public function upuserinfo()
	{
		...
		$upload->rootPath = './Public/Home/images/hportrait/';//上传目录
		...
		if ($info) {
			...
			foreach ($info as $key => $file) {
				if (!empty($file)) {
					$image->open('./Public/Home/images/hportrait/' . $file['savepath'] . $file['savename']);
					$width = $image->width();
					$height = $image->height();
					...
					$image->thumb($new_width, $new_height)->save('./Public/Home/images/hportrait/' . $file['savepath'] . "s_" . $file['savename']);
					unlink('./Public/Home/images/hportrait/' . $file['savepath'] . $file['savename']);
				}
			}
			$headimg = M('User')->where(array('id' => userid()))->getField('headimg');
			$headroot = PC_URL.'/Public/Home/images/hportrait/' . $file['savepath'] . "s_" . $file['savename'];
	...
	public function uppassword_mollymobi($oldpassword="", $newpassword="",$repassword="")
	{
		if (!userid()) {
			$this->error('请先登录！');
		}
	...
	public function uppaypassword_mollymobi($oldpaypassword, $newpaypassword, $repaypassword)
	{
		if (!userid()) {
			$this->error('请先登录！');
		}
	...
	public function upmoble_mollymobi($moble_new="", $moble_verify_new="")
	{
		if (!userid()) {
			$this->error('您没有登录请先登录！');
		}
	...
	public function upgoods_mollymobi($name="", $truename="", $idcard="", $moble="", $addr="", $paypassword="",$prov="",$city="")
	{
		if (!userid()) {
			redirect('/#login');
		}
