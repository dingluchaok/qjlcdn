...
class HomeController extends \Think\Controller
{
	protected function _initialize()
	{
		...
		if(WAP_URL !=""){
			...
			if(preg_match('/(iphone|android|Windows\sPhone)/i', $ua)){
				...
				if (isset($_GET['invit'])) {
					...
					if ($user['id']) {
						$mollymobi_redirect = WAP_URL."/Login/register/invit/".$user['id'];
					}else{
						$mollymobi_redirect = WAP_URL;
					}
				}else{
					$mollymobi_redirect = WAP_URL;
				}
				header("Location:".$mollymobi_redirect);
				die();
			} 
		}
		...
		//C('coin_menu_mollymobi',array('CNY','BTC','ETH'));
