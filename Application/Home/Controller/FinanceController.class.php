...
/* 		foreach ($CoinList as $k => $v) {
		...
		foreach ($CoinList as $k => $v) {
			...
			if ($v['name'] == 'cny') {
				...
				//开启市场时才显示对应的币
				if(in_array($v['name'],C('coin_on')) || 1){
					$coinList[$v['name']] = array('name' => $v['name'], 'img' => $v['img'], 'title' => $v['title'] . '(' . strtoupper($v['name']) . ')', 'xnb' => round($UserCoin[$v['name']], 6) * 1, 'xnbd' => round($UserCoin[$v['name'] . 'd'], 6) * 1, 'xnbz' => round($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd'], 6), 'jia' => $jia * 1, 'zhehe' => round(($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia, 2));
				}
				$cny['zj'] = round($cny['zj'] + (($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia), 2) * 1;
			}
		...
		$this->assign('cny', $cny);
		$this->assign('coinList', $coinList);//dump($coinList);
	...
	public function cindex() {
		...
		$CoinList = M('Coin')->where(array('status' => 1, 'c2c_show'=>0))->select();
		$UserCoin = M('UserCoin')->where(array('userid' => userid()))->find();
		$Market = M('Market')->where(array('status' => 1))->select();
		...
		foreach ($CoinList as $k => $v) {
			...
			if ($v['name'] == 'cny') {
				...
				//开启市场时才显示对应的币
				if(in_array($v['name'],C('coin_on')) || 1==1){
					$coinList[$v['name']] = array('id'=>$v['id'],'name' => $v['name'], 'img' => $v['img'], 'title' => $v['title'] . '(' . strtoupper($v['name']) . ')', 'xnb' => round($UserCoin[$v['name'].'c'], 6) * 1, 'xnbd' => round($UserCoin[$v['name'] . 'cd'], 6) * 1, 'xnbz' => round($UserCoin[$v['name'].'c'] + $UserCoin[$v['name'] . 'cd'], 6), 'jia' => $jia * 1, 'zhehe' => round(($UserCoin[$v['name'].'c'] + $UserCoin[$v['name'] . 'cd']) * $jia, 2));
				}
				$cny['zj'] = round($cny['zj'] + (($UserCoin[$v['name'].'c'] + $UserCoin[$v['name'] . 'cd']) * $jia), 2) * 1;
			}
		}
		$this->assign('cny', $cny);//dump($cny);
		$this->assign('coinList', $coinList);//dump($coinList);
		$this->assign('prompt_text', D('Text')->get_content('finance_index'));
		$this->display();
	}
	...
	public function payset() {
		if(empty($_POST)){
			...
			if($user['idcardauth'] == 0){
				$this->error('请先实名认证！', U('User/nameauth'));
			}
			...
			/* $c_bank_card = M('user')->where(array('id'=>userid()))->getField('c_bank_card');
				if(!empty($c_bank_card)) {
			$this->error('您已绑定');
			} */
			$input = I('post.');
			if(empty($input['code'])) {
			...
			if(session('realcard_verify') != $input['code']) {
				$this->error('短信验证码错误');
			}
			if(strlen($input['c_bank_card']) < 16 || strlen($input['c_bank_card']) > 22){
				$this->error('请输入正确的银行卡号');
			}
			if(empty($input['c_bank']) || empty($input['c_bank_detail'])){
				$this->error('请补全信息');
			}
			if($input['c_bank_card_again'] != $input['c_bank_card']){
				$this->error('确认卡号错误');
			}
			if(M('user')->where('id='.userid())->save($input) !== false){
				session('realcard_verify', null);
				$this->success('绑定成功');
			}else{
				$this->error('网络错误，请稍再试');
			}
		}
	}
	public function bindwechat() {
		if(IS_POST){
			$input = I('post.');
			if(empty($input['c_wechat_account']) || empty($input['c_wechat_img']) || empty($input['code'])) {
				$this->error('请补全信息');
			}
			if(!userid()){
				$this->error('请登录操作');
			...
			$input['c_wechat_img'] = $input['c_wechat_img'];
			if(M('user')->where('id='.userid())->save($input) !== false){
				session('realcard_verify', null);
				$this->success('绑定成功');
			}else{
	...
	public function bindalipay() {
		if(IS_POST){
			$input = I('post.');
			if(empty($input['c_alipay_account']) || empty($input['c_alipay_img']) || empty($input['code'])) {
				$this->error('请补全信息');
			}
			if(!userid()){
				$this->error('请登录操作');
			...
			$input['c_alipay_img'] = $input['c_alipay_img'];
			if(M('user')->where('id='.userid())->save($input) !== false){
				session('realcard_verify', null);
				$this->success('绑定成功');
			}else{
	...
	public function upbank($name, $bank, $bankprov, $bankcity, $bankaddr, $bankcard, $paypassword)
	{
		...
		if(strlen($bankcard) < 10 || strlen($bankcard) > 19){
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables mollymobi_mytx write , mollymobi_user_coin write ,mollymobi_finance write');
		$rs = array();
		...
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
		$rs[] = $finance_nameid = $mo->table('mollymobi_mytx')->add(array('userid' => userid(), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function mytxChexiao($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables mollymobi_user_coin write,mollymobi_mytx write,mollymobi_finance write');
		$rs = array();
		$finance = $mo->table('mollymobi_finance')->where(array('userid' => $mytx['userid']))->order('id desc')->find();
		$finance_num_user_coin = $mo->table('mollymobi_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
		$rs[] = $mo->table('mollymobi_mytx')->where(array('id' => $mytx['id']))->setField('status', 2);
		$finance_mum_user_coin = $mo->table('mollymobi_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$finance_hash = md5($mytx['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mytx['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.mollymobi.com');
		$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
	...
	public function myzr($coin = NULL)
	{
		...
		$mollymobi_getCoreConfig = mollymobi_getCoreConfig();
		if(!$mollymobi_getCoreConfig){
			$this->error('核心配置有误');
		}

		$this->assign("mollymobi_opencoin",$mollymobi_getCoreConfig['mollymobi_opencoin']);
		
		if($mollymobi_getCoreConfig['mollymobi_opencoin'] == 1)
		{	
				//dump(111);	
				...
				if (!$Coin['zr_jz']) {
					...
					$qbdz = $coin . 'b';
                    if($Coin['hand_zr_address']) {
                    	//dump(333);
				$qianbao = $Coin['hand_zr_address'];
			}else{
					if (!$user_coin[$qbdz]) {
						if ($Coin['type'] == 'rgb') {
				...
				if (!$Coin['zr_jz']) {
					...
					$qianbao = $Coin['mollymobi_coinaddress'];
	...
	public function upmyczrgb($num, $coin, $address){
		if (!userid()) {
			redirect("/#login");
		}
		...
		check_server();
		if($num <= 0){
			$this->error('请输入正确的转入数量');
		}
		for (; ; ) {
			$tradeno = tradeno();
	
			if (!M("Myzr")->where(array("tradeno" => $tradeno))->find()) {
				break;
			}
		}
		//dump($tradeno);
		$Coin = M("Coin")->where(array("name" => $coin))->find();
		$mycz = M('Myzr')->add(array('userid' => userid(), 'username'=>$Coin['hand_zr_address'], 'num' => $num, 'addtime' => time(), 'tradeno' => $tradeno, 'coinname'=>$coin, 'status'=>0, 'fee'=>0, 'mum'=>0, 'zr_address'=>$address));
		//dump($Coin);
		if ($mycz) {
			$this->success('转入成功', array("id" => $mycz, 'tradeno'=>$tradeno));
		}
		else {
			$this->error('转入失败');
		}
	}
	public function myczHuikuan2($id = NULL)
	{
		if (!userid()) {
			$this->error('请先登录！');
		}
		...
		$mycz = M('Myzr')->where(array('id' => $id))->find();
		...
		$rs = M('Myzr')->where(array('id' => $id))->save(array('status' => 3));
	...
	public function myccoininlog($coin = NULL){
	...
	public function myccoinoutlog($coin = NULL){
	...
	public function ccoinin($coin = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
	...
	public function ccoinout($coin = NULL)
	{
		if (!userid()) {
			redirect('/#login');
		}
		...
		$user_coin = M('UserCoin')->where(array('userid' => userid()))->find();
		$user_coin[$coin.'c'] = round($user_coin[$coin.'c'], 6);
		$this->assign('user_coin', $user_coin);
		$this->assign('coin_list', $coin_list);
		$this->assign('xnb', $coin);
		$this->assign('xnbc', $coin.'c');
		$this->display();
	}
	public function upmyccoin($coin, $num, $paypassword, $type) {
		if (!userid()) {
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables  mollymobi_user_coin write  , mollymobi_ccoin_log write');
		$rs = array();
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
		if(empty($usercoin[$coin.'c'])){
			$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setField($coin.'c', $num);
		}else{
			$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setInc($coin.'c', $num);
		}
		$rs[] = $mo->table('mollymobi_ccoin_log')->add(array(
				'uid' => userid(),
				'coin' => $coin,
				'amount' => $num,
				'addtime' => time(),
	...
	public function upmyccoinout($coin, $num, $paypassword, $type) {
		if (!userid()) {
			$this->error('您没有登录请先登录！');
		}
		...
		$mo->execute('lock tables  mollymobi_user_coin write  , mollymobi_ccoin_log write');
		$rs = array();
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setDec($coin.'c', $num);
		$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setInc($coin, $num);
		$rs[] = $mo->table('mollymobi_ccoin_log')->add(array(
				'uid' => userid(),
				'coin' => $coin,
				'amount' => $num,
	...
	public function upmyzc($coin, $num, $addr, $paypassword, $moble_verify)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables  mollymobi_user_coin write  , mollymobi_myzc write  , mollymobi_myzr write , mollymobi_myzc_fee write');
			$rs = array();
			$rs[] = $mo->table("mollymobi_user_coin")->where(array("userid" => userid()))->setDec($coin, $mum);
			$rs[] = $mo->table("mollymobi_user_coin")->where(array("userid" => userid()))->setInc($coin.'d', $mum);

			if ($fee) {
				if ($mo->table('mollymobi_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
					$rs[] = $mo->table('mollymobi_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
				else {
					$rs[] = $mo->table('mollymobi_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
			}

			$rs[] = $mo->table('mollymobi_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 0));
			//$rs[] = $mo->table('mollymobi_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));

			if ($fee_user) {
				//$rs[] = $mo->table('mollymobi_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $Coin['zc_user'] . time()), 'num' => $num, 'fee' => $fee, 'type' => 1, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			}
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('mollymobi_user_coin')->where(array($qbdz => $addr))->find()) {
				$peer = M('UserCoin')->where(array($qbdz => $addr))->find();
				...
				$rs = array();
				$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);
				...
				$rs[] = $mo->table('mollymobi_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
				$rs[] = $mo->table('mollymobi_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
				...
				$mo->execute('lock tables  mollymobi_user_coin write  , mollymobi_myzc write ,mollymobi_myzr write, mollymobi_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('mollymobi_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $aid = $mo->table('mollymobi_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));

				if ($fee && $auto_status) {
					$rs[] = $mo->table('mollymobi_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'type' => 2, 'addtime' => time(), 'status' => 1));

					if ($mo->table('mollymobi_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
						$rs[] = $r = $mo->table('mollymobi_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
						debug(array('res' => $r, 'lastsql' => $mo->table('mollymobi_user_coin')->getLastSql()), '新增费用');
					}
					else {
						$rs[] = $r = $mo->table('mollymobi_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					}
				}
	...
	public function myad($type,$state=1)
	{
		...
		foreach ($list as $k => $v) {
			...
			$deal_num = M('order_buy')->where(array('sell_sid'=>$v['id'], 'status'=>array('neq', 5)))->sum('deal_num');
			$deal_num = $deal_num ? $deal_num : 0;
			$fee_num = M('order_buy')->where(array('sell_sid'=>$v['id'], 'status'=>array('neq', 5)))->sum('fee');
			$fee_num = $fee_num ? $fee_num : 0;
			$list[$k]['deal'] = $deal_num + $fee_num;
	...
	public function apply_seller() {
		if(empty($_POST)) {
			...
			$m = M('seller_apply');
			$find = $m->where(array('uid'=>userid(), 'status'=>0))->find();
			if(!empty($find)) {
				$this->error('您还有申请未审核，请等待');
			}
			$coin = C('seller_desc_coin');
			$amount = C('seller_desc_amount');
			$days = C('seller_cancel_days');
			if(!$coin || !$amount || !$days) {
				$this->error('参数不全，请联系管理员设置');
			}
			$user_coin = M('user_coin')->where(array('userid'=>userid()))->find();
			...
			$mo = M();
			$mo->startTrans();
			$rs1 = $mo->table('mollymobi_user_coin')->where(array('userid'=>userid()))->setDec($coin, $amount);
			$rs2 = $mo->table('mollymobi_user_coin')->where(array('userid'=>userid()))->setInc($coin.'d', $amount);
			$rs3 = $mo->table('mollymobi_seller_apply')->add(array(
					'uid'=>userid(),'coin'=>$coin,'amount'=>$amount,'time'=>time()
			));
			if($rs1 && $rs2 && $rs3) {
				$mo->commit();
				$this->success('申请成功');
	...
	public function cancel_seller() {
		if(IS_POST) {
			$id = I('post.id');
			$find = M('seller_apply')->where(array('id'=>$id))->find();
			if(empty($find)) {
				$this->error('参数错误');
			}
			if($find['status'] != 1) {
			...
			$days = C('seller_cancel_days');
			if(($days*86400+$find['time']) > time()) {
				$this->error('自申请起'.$days.'天后可取消');
			}
			$mo = M();
			$mo->startTrans();
			$rs1 = $mo->table('mollymobi_user_coin')->where(array('userid'=>userid()))->setDec($find['coin'].'d', $find['amount']);
			$rs2 = $mo->table('mollymobi_user_coin')->where(array('userid'=>userid()))->setInc($find['coin'], $find['amount']);
			$rs3 = $mo->table('mollymobi_seller_apply')->where(array('id'=>$id))->save(array(
					'status'=>3
			));
			if($rs1 && $rs2 && $rs3) {
				$mo->commit();
