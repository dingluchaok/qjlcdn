...
class LoginController extends HomeController
{
	...
	public function upregister($username='', $password='', $repassword='', $verify='', $invit='', $moble='', $moble_verify='')
	{
		...
		$rs = array();
		$rs[] = $mo->table('mollymobi_user')->add(array('username' => $username, 'moble' => $moble, 'mobletime' => time(), 'password' => md5($password), 'invit' => $tradeno, 'tpwdsetting' => 1, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1));
	...
	public function test() {
		//M('user_coin')->add(array('userid' => 1514));
	}
	...
	public function realad()
	{
		...
		if (!check($user['moble'], 'moble')) {
			$this->error('手机号码格式错误！');
		}
	
		$code = rand(111111, 999999);
		session('realad_verify', $code);
		$content = '您正在进行手机操作，您的验证码是' . $code;
	
		if (send_moble($user['moble'], $content)) {
			if(MOBILE_CODE ==0 ){
				$this->success('您的验证码是'.$code);
			}else{
				$this->success('验证码已发送');
	...
	public function submit($username="", $password="",$moble="", $verify = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables mollymobi_user write , mollymobi_user_log write ');
		$rs = array();
		$rs[] = $mo->table('mollymobi_user')->where(array('id' => $user['id']))->setInc('logins', 1);
		
		$rs[] = $mo->table('mollymobi_user')->where(array('id' => $user['id']))->save(array('token'=>$token_user));
		
		$rs[] = $mo->table('mollymobi_user_log')->add(array('userid' => $user['id'], 'type' => '登录', 'remark' => $remark, 'addtime' => $logintime, 'addip' => $ip, 'addr' => get_city_ip(), 'status' => 1));
	...
	public function findpwd()
	{
		if (IS_POST) {
			...
			if(M_ONLY==0){
				...
				$mo->execute('lock tables mollymobi_user write , mollymobi_user_log write ');
				$rs = array();
				$rs[] = $mo->table('mollymobi_user')->where(array('id' => $user['id']))->save(array('password' => md5($input['password'])));
	...
	public function password_up($password=""){
		...
		$mo->execute('lock tables mollymobi_user write , mollymobi_user_log write ');
		$rs = array();
		$rs[] = $mo->table('mollymobi_user')->where(array('moble' => $user['moble']))->save(array('password' => md5($password)));
	...
	public function findpaypwd()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables mollymobi_user write , mollymobi_user_log write ');
			$rs = array();
			$rs[] = $mo->table('mollymobi_user')->where(array('id' => $user['id']))->save(array('paypassword' => md5($input['password'])));
			...
			if (check_arr($rs)) {
				...
				$mo->execute('rollback');
				$this->error('修改失败' . $mo->table('mollymobi_user')->getLastSql());
			}
		}
		else {
			$this->display();
