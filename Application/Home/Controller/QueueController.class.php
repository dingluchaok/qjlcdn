...
*/	public function index()
	...
	public function checkYichang()
	{
		...
		$mo->execute('lock tables mollymobi_trade write');
		$Trade = M('Trade')->where('deal > num')->order('id desc')->find();

		if ($Trade) {
			if ($Trade['status'] == 0) {
				$mo->table('mollymobi_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num']), 'status' => 1));
			}
			else {
				$mo->table('mollymobi_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num'])));
			}
		...
		foreach ($coinList as $k => $v) {
			...
			foreach ($listtransactions as $trans) {
				...
				if ($trans['category'] == 'receive') {
					...
					$rs = array();
					$rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $user['id']))->setInc($coin, $trans['amount']);

					if ($res = $mo->table('mollymobi_myzr')->where(array('txid' => $trans['txid']))->find()) {
						echo 'mollymobi_myzr find and set status 1';
						$rs[] = $mo->table('mollymobi_myzr')->save(array('id' => $res['id'], 'addtime' => time(), 'status' => 1));
					}
					else {
						echo 'mollymobi_myzr not find and add a new mollymobi_myzr' . "\n";
						$rs[] = $mo->table('mollymobi_myzr')->add(array('userid' => $user['id'], 'username' => $trans['address'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['txid'], 'num' => $true_amount, 'mum' => $trans['amount'], 'addtime' => time(), 'status' => 1));
					}
	...
	public function upsssss(){
		$c = array('eth_usdt', 'gicc_usdt', 'neo_usdt', 'trx_usdt', 'etc_usdt', 'xrp_usdt', 'eos_usdt', 'btc_usdt', 'ltc_usdt', 'etc_usdt');
		foreach($c as $k=>$v){
			$aaaa = $this->upTrade($v);
			echo $aaaa;
		}
	...
	public function upTrade_mollymobi_8a201aa602cd9448($market = NULL)
	{
		echo rand();
		$type = rand(1, 2);
		...
		$url = 'https://coinbig.org/api/publics/vip/ticker?symbol='.$xnb.'_'.$rmb;
		$content = file_get_contents($url);
		$content = json_decode($content, true);
		print_r($content);
		$wei = 1000;
		
		if(floatval($content['data']['ticker']['buy'])<10){
			$wei = 100000;
		}
		
		if(floatval($content['data']['ticker']['sell'])<10){
			$wei = 100000;
		}

		$min_price = floatval($content['data']['ticker']['buy'])*$wei;
		$max_price = floatval($content['data']['ticker']['sell'])*$wei;
		...
		if ($type == 1) {
			$rs[] = $mo->table('mollymobi_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('mollymobi_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
			$mo->execute('unlock tables');
	...
	public function huilvgenxin(){
		$time=time();
		$coin_arr = array('Btc'=>'bitcoin','Eth'=>'ethereum');
