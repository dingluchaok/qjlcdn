...
class AdsController extends CommonController {
    ...
    public function click($hash,$aid){
        ...
        $mo->execute('lock tables mollymobi_app_log write ,mollymobi_user_coin write');

        $rs[] = $mo->table('mollymobi_user_coin')->where(array('userid' => $uid))->setDec($price_coin['name'], $Vip['price_num']);
        $rs[] = $mo->table('mollymobi_app_log')->add(
            array(
                'uid'     => $uid,
                'type'    => 'click_ads',
