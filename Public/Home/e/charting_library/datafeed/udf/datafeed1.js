...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, socket, updateFrequency) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.defaultConfiguration = function() {
	return {
		...
		supported_resolutions:["1","5","15","30","60","120","240","480","D","W"],
...
Datafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function(callback) {
	if (this._configuration.supports_time) {
		this._send('time', {}, function(response) {
				var time = +response;
				if (!isNaN(time)) {
					callback(time);
				}
...
Datafeeds.UDFCompatibleDatafeed.prototype._send = function(cmd, params, callback)
{
	var jsonStr = "chart|" + cmd + "|" + JSON.stringify(params);
	this._socket.send(jsonStr, callback);
...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
	...
	//		alert(this._datafeedURL + '/config');
	//		that._setupWithConfiguration(that.defaultConfiguration());
	//	});
};
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	this._send(this._historyURL, param, function(response) {
		var data = parseJSONorNot(response);
